...
The DM SHALL be able to send a `start_combat` action that transitions the room into active combat, sets the round counter, and establishes the first active turn. Before allowing combat to start, the server SHALL verify that every player and companion entity in the room has a non-null initiative value.

#### Scenario: DM starts combat — all initiatives set
- **WHEN** a DM-role client sends `{ "type": "start_combat" }` and every player and companion entity in the room has a non-null initiative
...
- **WHEN** a DM-role client sends `start_combat` and one or more player or companion entities have `initiative: null`
