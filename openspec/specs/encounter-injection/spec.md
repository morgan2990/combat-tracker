...
The DM Panel SHALL render an "Encounter Templates" control that fetches `GET /api/encounters?edition=<room's edition>` and lists the DM's saved encounters matching the room's edition. Selecting one SHALL send an `inject_encounter` WS message with the encounter's `id`.

#### Scenario: Dropdown lists only matching-edition encounters
- **WHEN** the DM Panel opens the Encounter Templates control in a `"5e"` room and the DM owns encounters in both `"5e"` and `"5.5e"`
- **THEN** only the `"5e"` encounters appear in the list

#### Scenario: Selecting an encounter dispatches injection
- **WHEN** the DM selects an encounter from the dropdown
