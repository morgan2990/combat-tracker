...
Provides server-side storage and ownership-scoped CRUD for DM-authored encounter blueprints — reusable groups of monsters (official and/or custom) that a DM can save, edit, and later inject into a live room. Mirrors the `CustomMonster` repository pattern: encounters are stored in a dedicated MongoDB collection, scoped to their creating DM via `owner_id`, and exposed through a standard set of authenticated REST endpoints.
