...
The DM SHALL be able to send a `start_combat` action that transitions the room into active combat, sets the round counter, and establishes the first active turn.

#### Scenario: DM starts combat
- **WHEN** a DM-role client sends `{ "type": "start_combat" }`
